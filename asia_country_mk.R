#  acled_asia_country_mk.R
#
#    Script to take output from acled_asia_importer.R and pull data for a specific 
...
#
asia_country <- function(events_df, c = "Myanmar"){
     ...
     events.country <- events_df %>%
          ...
          mutate(month = as.numeric(substr(event_date, 6, 7))) %>%
          mutate(day = as.numeric(substr(event_date, 9, 10))) %>%
...
#    - groups by actor
asia_cntry_recent_by_actor <- function(country_tbl, fdate) {
...
# be filtered out.
asia_cntry_remove_ongoing <- function(country_tbl, actor, actor_num){
...
asia_country_mk <- function(events_df, c="Myanmar", rem_actor_name="",
                             rem_actor_num=0){
     # Get country code for the name
     c_code <- countrycode(c, "country.name", "cown", warn=FALSE)
     ...
     # Since these are monthly files, just do past 13 months from today's date
     thismonth <- month(today())
     thisyear <- year(today())

     file_date <- str_c(thisyear,thismonth,"1",sep="-")
     
     
     # Filter to the most recent year, plus up to one month    
     events.recent <- asia_cntry_recent_by_actor(events.country.fatal, file_date)
     ...
     if(rem_actor_num[1] > 0){
          events.wo <- asia_cntry_remove_ongoing(events.recent, rem_actor_name, 
                                                  rem_actor_num)
     } else {
