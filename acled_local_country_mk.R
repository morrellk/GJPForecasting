...
#
#    Script to take output from a local processing file and pull data 
#    broken down by actor. Modified from acled_country_mk.R to process local
#    data - using first with Burundi.
#
...
#    Functions:
#         Main:
#         acled_local_country_mk(events_df, c="Nigeria", rem_actor_name=c("Boko Haram","Boko Haram"),
#                             rem_actor_num=c(1,2))
#         Supporting:
...
acled_local_country_mk <- function(events_df, c="Nigeria", rem_actor_name="",
                             rem_actor_num=0){
     # Get country code for the name
     c_code <- countrycode(c, "country.name", "cown", warn=FALSE)
     ...
     events.country <- events_df %>%
          ...
          select(., event_id_burundi, event_date, year, month, day, time_precision, event_type, 
     ...
     # Determine end date of file - use latest event (***At present - name is specific)
     last_event <- filter(events_df, event_id_burundi == max(event_id_burundi))
     last_date <- dmy(last_event$event_date)
     file_date <- str_c(year(last_date),month(last_date),day(last_date), sep="_")
     ...
     # do for actor 2
     # Need to deal with NA's in actor 2 - most of which have a value in ally-actor 2
     events.wo <- events.wo %>%
          ungroup(.)
     events.wo$actor2 <-sapply( c(1:length(events.wo$actor2)), function(x) {
          if (is.na(events.wo$actor2[x])) 
               {events.wo$actor2[x] <- events.wo$ally_actor_2[x]}
          else
               events.wo$actor2[x] <- events.wo$actor2[x]
               })
     ...
     filename <- str_c(c, "local", file_date, sep="_")
